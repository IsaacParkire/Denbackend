...
class Command(BaseCommand):
    help = 'Populate membership plans with default data'
    ...
    def handle(self, *args, **options):
        ...
        if created:
            self.stdout.write(self.style.SUCCESS('✓ Premium plan created'))
        else:
            self.stdout.write('• Premium plan already exists')

        # VIP Elite Membership Plan
        ...
        if created:
            self.stdout.write(self.style.SUCCESS('✓ VIP Elite plan created'))
        else:
            self.stdout.write('• VIP Elite plan already exists')

        self.stdout.write(
            self.style.SUCCESS(
                f'\nSuccessfully set up membership plans! Total plans: {MembershipPlan.objects.count()}'
            )
        )
